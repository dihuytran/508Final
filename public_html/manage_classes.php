...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_class'])) {
    ...
    $classType = filter_input(INPUT_POST, 'classType', FILTER_UNSAFE_RAW);

    $dateTime = $classDate . ' ' . $classTime;

    $sql = "INSERT INTO class (class_name, capacity, class_date, class_time, class_type) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([htmlspecialchars($className), $capacity, $classDate, $dateTime, htmlspecialchars($classType)]);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_class'])) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->execute([htmlspecialchars($className), $capacity, $classDate, $dateTime, $classId]);
}
    ...
    <div class="row">
        ...
        <div class="col-md-6 form-column">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>Class Time</label>
                    <input type="time" name="classTime" class="form-control">
                </div>
                <div class="form-group">
