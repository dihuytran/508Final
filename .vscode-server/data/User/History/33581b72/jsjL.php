...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Debug: Print email and password to check if they are captured correctly
    echo "Email: " . $email . "<br>";
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            switch ($_SESSION['user_role']) {
                case 'guest':
                    ...
                    exit;
                case 'staff':
                case 'admin': // Admin and staff might need different handling if different dashboards are required
                    $stmt = $conn->prepare("SELECT staff_role FROM staff WHERE email = :email"); // Further refine role if necessary
                    $stmt->bindParam(':email', $email, PDO::PARAM_STR);
                    $stmt->execute();
