...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_equipment'])) {
    ...
    $sql = "INSERT INTO equipment (inventory_id, renter_id) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$inventoryId, $renterId]);
}

// Fetch equipment for display
$stmt = $conn->prepare("SELECT equipment_id, inventory_id, renter_id FROM equipment");
$stmt->execute();
            ...
            <tbody>
                <?php foreach ($equipment as $item) { ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($item['renter_id']); ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
