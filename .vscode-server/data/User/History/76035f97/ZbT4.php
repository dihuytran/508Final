...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reserve'])) {
    ...
    if ($inventoryId) {
        ...
        if ($quantityAvailable > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO equipment (inventory_id, renter_name, type) 
            SELECT ?, ?, type FROM inventory WHERE inventory_id = ?");
            $stmt->execute([$inventoryId, $userId, $inventoryId]);
