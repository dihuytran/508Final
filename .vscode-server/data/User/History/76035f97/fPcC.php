...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['return'])) {
    ...
    if ($inventoryId) {
        // Remove the reservation from the equipment table
        $stmt = $conn->prepare("DELETE FROM equipment WHERE inventory_id = ? AND renter_name = ?");
        $stmt->execute([$inventoryId, $userId]);
                ...
                <select name="inventory_id" id="inventory_id" class="form-control" required>
                    ...
                    <?php foreach ($inventory as $item): ?>
                        <option value="<?php echo $item['inventory_id']; ?>">
                            Inventory ID: <?php echo $item['inventory_id']; ?> - Quantity Available: <?php echo $item['quantity_available']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
